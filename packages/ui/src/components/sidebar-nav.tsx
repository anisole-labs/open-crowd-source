...
export type DocsSidebarNavProps = {
  mainNavItems: NavItemWithChildren[];
};

export function DocsSidebarNav({ mainNavItems }: DocsSidebarNavProps) {
    ...
    <div className="w-full">
      {mainNavItems.map((mainNavItem, index) => (
        <div key={index} className={cn("pb-4")}>
          ...
          {mainNavItem?.childrenItems?.length && (
            <DocsSidebarNavItems
              items={mainNavItem.childrenItems}
              pathname={pathname}
            />
          )}
        </div>
